...
export function Hero({ onOpenChat }: HeroProps) {
  ...
  // Feathered proximity field for the hidden words. Each carries a per-frame
  // --near value (0..1) computed from distance to the smoothed cursor with a
  // smoothstep falloff; once mostly uncovered they are marked .lit and keep
  ...
  useEffect(() => {
    ...
    measure();
    // Word centers shift once the webfonts land, and again if the font
    // tester swaps the display face; re-measure so they aren't stuck at
    // fallback-font positions.
    document.fonts?.ready.then(measure);
    window.addEventListener("fontchange", measure);
    const ro = new ResizeObserver(measure);
    ro.observe(section);

    // Full reveal at the cursor, fading to nothing at REVEAL_RADIUS; found
    // once mostly uncovered. Generous so the words start glowing while the
    // cursor is still a fair distance away and first-time visitors can't
    // miss them.
    ...
    return () => {
      ...
      window.removeEventListener("fontchange", measure);
      ro.disconnect();
